...
namespace LasMarias.Dataservice
{
    public class Artikel
    {
        ...
        protected const string COLUMNS = "artikel_id, name,ekpreis, vkpreis, anlagedatum, preis";
        ...
        public Artikel(NpgsqlDataReader reader)
        {
            ...
            this.ekpreis = reader.IsDBNull(2) ? (double?)null : reader.GetDouble(2);
            this.vkpreis = reader.IsDBNull(3) ? (double?)null : reader.GetDouble(3);
            this.AnlageDatum = reader.IsDBNull(4) ? (DateTime?)null : reader.GetDateTime(4);
            this.Preis = reader.IsDBNull(5) ? (double?)null : reader.GetDouble(5);
       ...
       public Datei Bild { get; set; }
        ...
        public int Save(NpgsqlConnection connection)
        {
            ...
            {
                command.CommandText = $"update {TABLE} set name = :name, ekpreis = :ekpreis, vkpreis = :vkpreis, anlagedatum = :anlagedatum, preis = :preis where artikel_id = :artikelid";
            }
            else // insert
            {
                command.CommandText = $"select nextval('{TABLE}_seq')";
                this.ArtikelId = (long)command.ExecuteScalar();
                command.CommandText = $"insert into {TABLE} ({COLUMNS}) values (:artikelid, :name,  :ekpreis, :vkpreis, :anlagedatum ,:preis)";
            }
        ...
        public int Delete(NpgsqlConnection cnn)
        {
            ...
            try
            {
              ...
              //  }
                ...
                cmd.CommandText = "delete from lasmarias.artikel where artikel_id = :aid";
