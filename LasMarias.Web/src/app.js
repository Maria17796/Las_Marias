...
export default class Application {
  ...
  constructor() {
    ...
    const toolbarData = localStorage.getItem('rolle');
    if (toolbarData) {
      ...
      if(toolbarOptions === 3)
        ...
        if (toolbarData) {
          const toolbarOptions = JSON.parse(toolbarData);
          if(toolbarOptions === 0)
            this.toolbar = new toolbarCustumer({
              app: this,
              target: header,
              benutzer: toolbarOptions
            });
...
if (hashParts.length > 1) {
  ...
  apiGetArtikelDatei(successCallback, errorCallback, id) {
    ...
    fetch(this.#apiUrl + "/person/login", {
     ...
     // cache: "no-cache",
      ...
      .then((r) => {
        console.log(r);
        if (r.status == 200) {
       return r.json(); // Wandelt den JSON-String in ein JavaScript-Objekt um
        }
        else throw new Error(r.status + " " + r.statusText);
      })
